...
#gsed -i 's/LANGS .*/LANGS = '"$makelanguages"'/' Makefile.am
#
...
cat $PluginName-py.pot $PluginName-xml.pot | msguniq --no-wrap --no-location -o $PluginName.pot -
OLDIFS=$IFS
IFS=" "
for lang in "${languages[@]}" ; do
	if [ -f $lang.po ]; then
		printf "Updating existing translation file %s.po\n" $lang
		msgmerge --backup=none --no-wrap --no-location -s -U $lang.po $PluginName.pot && touch $lang.po;
